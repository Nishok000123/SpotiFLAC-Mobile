...
mod sys {
    ...
    // Bionic exposes statx starting at API 30. Older Android app seccomp
    // policies can kill the process on the raw syscall, so a missing libc
    // symbol must return ENOSYS and use rustix's existing stat64 fallback.
    #[cfg(target_os = "android")]
    weakcall! {
        pub(super) fn statx(
            dirfd_: BorrowedFd<'_>,
